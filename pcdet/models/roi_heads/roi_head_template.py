...
class RoIHeadTemplate(nn.Module):
    ...
    def get_box_reg_layer_loss(self, ret_dict):
        loss_cfgs = self.model_cfg.LOSS_CONFIG
        code_size = self.box_coder.code_size
        reg_valid_mask = ret_dict['reg_valid_mask'].view(-1)
        gt_boxes3d_ct = ret_dict['gt_of_rois'][..., 0:code_size]
        gt_of_rois_src = ret_dict['gt_of_rois_src'][..., 0:code_size].view(-1, code_size)
        rcnn_reg = ret_dict['rcnn_reg']  # (rcnn_batch_size, C)
    ...
    def get_box_cls_layer_loss(self, ret_dict):
        loss_cfgs = self.model_cfg.LOSS_CONFIG
        rcnn_cls = ret_dict['rcnn_cls']
        rcnn_cls_labels = ret_dict['rcnn_cls_labels'].view(-1)
    ...
    def get_loss(self, ret_dict, tb_dict=None):
        ...
        rcnn_loss = 0
        rcnn_loss_cls, cls_tb_dict = self.get_box_cls_layer_loss(ret_dict)
        rcnn_loss += rcnn_loss_cls
        tb_dict.update(cls_tb_dict)

        rcnn_loss_reg, reg_tb_dict = self.get_box_reg_layer_loss(ret_dict)
