...
class VoxelSetAbstraction(nn.Module):
    def __init__(self, model_cfg, voxel_size, point_cloud_range, num_bev_features=None,
        ...
        self.flow_vsa_point_feature_fusion = nn.Sequential(
            nn.Linear(c_in, self.model_cfg.NUM_OUTPUT_FEATURES, bias=False),
            nn.BatchNorm1d(self.model_cfg.NUM_OUTPUT_FEATURES),
            nn.ReLU(),
        )
    ...
    def forward(self, batch_dict):
        ...
        # 对关键点,利用不同层的voxel center feature进行特征聚合
        ...
        # 通过mlp对拼接的特征进行融合
        det_point_features = self.vsa_point_feature_fusion(point_features.view(-1, point_features.shape[-1]))
        flow_point_feature = self.flow_vsa_point_feature_fusion(point_features.view(-1, point_features.shape[-1]))

        batch_dict['point_features'] = det_point_features  # (BxN, C)
        batch_dict['flow_point_feature'] = flow_point_feature  # (BxN, C)
