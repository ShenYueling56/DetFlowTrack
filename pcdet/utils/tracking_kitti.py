...
def get_objects_from_label(label_file):
    ...
    frame_objects = []
    for line in lines:
        object = Tracking_Object3d(line)
        if (object.frame_idx==frame_idx):
            frame_objects.append(object)
        else:
            seq_objects.append(frame_objects)
            frame_objects = []
            frame_idx = frame_idx + 1
            while (object.frame_idx  > frame_idx):
                seq_objects.append(frame_objects)
                frame_idx = frame_idx + 1
            frame_objects.append(object)
    # for last frame
...
class Tracking_Object3d(object):
    def __init__(self, line):
        label = line.strip().split(' ')
        self.src = line
        self.frame_idx = int(label[0])
        self.track_idx = int(label[1])
    ...
    def set_after_trans(self, location, rotation_y, bbox):
        self.loc = location
        self.ry = rotation_y
        self.bbox = bbox
        return self
