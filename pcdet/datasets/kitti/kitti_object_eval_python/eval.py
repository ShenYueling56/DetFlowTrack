...
def calculate_iou_partly(gt_annos, dt_annos, metric, num_parts=50):
    ...
    total_dt_num = np.stack([len(a["name"]) for a in dt_annos if not a==None], 0)
    total_gt_num = np.stack([len(a["name"]) for a in gt_annos if not a==None], 0)
    ...
    for num_part in split_parts:
        ...
        if metric == 0:
            gt_boxes = np.concatenate([a["bbox"] for a in gt_annos_part if not a== None], 0)
            dt_boxes = np.concatenate([a["bbox"] for a in dt_annos_part if not a==None], 0)
            overlap_part = image_box_overlap(gt_boxes, dt_boxes)
        elif metric == 1:
            loc = np.concatenate(
                [a["location"][:, [0, 2]] for a in gt_annos_part if not a==None], 0)
            dims = np.concatenate(
                [a["dimensions"][:, [0, 2]] for a in gt_annos_part if not a==None], 0)
            rots = np.concatenate([a["rotation_y"] for a in gt_annos_part if not a==None], 0)
            gt_boxes = np.concatenate(
                [loc, dims, rots[..., np.newaxis]], axis=1)
            loc = np.concatenate(
                [a["location"][:, [0, 2]] for a in dt_annos_part if not a==None], 0)
            dims = np.concatenate(
                [a["dimensions"][:, [0, 2]] for a in dt_annos_part if not a==None], 0)
            rots = np.concatenate([a["rotation_y"] for a in dt_annos_part if not a==None], 0)
            dt_boxes = np.concatenate(
                [loc, dims, rots[..., np.newaxis]], axis=1)
            overlap_part = bev_box_overlap(gt_boxes, dt_boxes).astype(
                np.float64)
        elif metric == 2:
            loc = np.concatenate([a["location"] for a in gt_annos_part if not a==None], 0)
            dims = np.concatenate([a["dimensions"] for a in gt_annos_part if not a==None], 0)
            rots = np.concatenate([a["rotation_y"] for a in gt_annos_part if not a==None], 0)
            gt_boxes = np.concatenate(
                [loc, dims, rots[..., np.newaxis]], axis=1)
            loc = np.concatenate([a["location"] for a in dt_annos_part if not a==None], 0)
            dims = np.concatenate([a["dimensions"] for a in dt_annos_part if not a==None], 0)
            rots = np.concatenate([a["rotation_y"] for a in dt_annos_part if not a==None], 0)
            dt_boxes = np.concatenate(
                [loc, dims, rots[..., np.newaxis]], axis=1)
...
def eval_class(gt_annos,
    ...
    """
    print("gt len ", str(len(gt_annos)), "dt len: ", str(len(dt_annos)))
    assert len(gt_annos) == len(dt_annos)
    num_examples = len(gt_annos)
    split_parts = get_split_parts(num_examples, num_parts)
    for i, g in enumerate(gt_annos):
        if g == None:
            print("None -2", str(i))
