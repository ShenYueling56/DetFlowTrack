...
class KittiDataset(DatasetTemplate):
    ...
    def __getitem__(self, index):
        ...
        data_dict, aug_dict = self.prepare_data(data_dict=input_dict)
...
if __name__ == '__main__':
    ...
    if sys.argv.__len__() > 1 and sys.argv[1] == 'create_kitti_infos':
        ...
        # ROOT_DIR = Path("/media/shenyl/Elements/KITTI_object/object/")
        ...
        # yaml_path = "/home/shenyl/Documents/code/SSDT/tools/cfgs/dataset_configs/kitti_dataset.yaml"
        f_yaml = open(sys.argv[1], encoding="UTF-8")
        cfg = EasyDict(yaml.load(f_yaml, Loader=yaml.FullLoader))
        # 设置root_path
        # root_path = Path("/media/shenyl/Elements/KITTI_object/object/")
        root_path = Path(cfg.DATA_PATH)
        train_dataset = KittiDataset(cfg, ['Car', 'Pedestrian', 'Cyclist'], training=True, root_path=root_path,
                                      logger=None)

        print(list(DataLoader(train_dataset, batch_size=1, shuffle=True)))
